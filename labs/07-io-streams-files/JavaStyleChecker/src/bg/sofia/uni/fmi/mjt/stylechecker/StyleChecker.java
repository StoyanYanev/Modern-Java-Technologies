...
public class StyleChecker {
    ...
    private List<String> findAllViolations(String line) {
        ...
        if (line.matches("^([^;]+[;]){2,}$")) {
            violations.add(ViolationsUtils.ONE_STATEMENT_PER_LINE);
        }
        if (line.matches("^import .+\\*;$")) {
            violations.add(ViolationsUtils.WILDCARDS_NOT_ALLOWED);
        }
        if (line.matches("^\\{.*$")) {
            violations.add(ViolationsUtils.OPEN_BRACKETS_ON_SAME_LINE);
        }
        if (line.matches("^package .*[A-Z_].*")) {
            violations.add(ViolationsUtils.INVALID_PACKAGE_NAME);
        }
        if (line.matches("^(?!import).*") && line.length() > MAX_LENGTH_LINE) {
            violations.add(ViolationsUtils.LINE_LENGTH_NOT_EXCEED);
        }
